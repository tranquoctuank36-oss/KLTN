    ...
    <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          ...
          {imagePreviews.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
              {imagePreviews.map((preview, index) => (
                <div
                  key={index}
                  className="relative w-full aspect-square rounded-lg overflow-hidden border border-gray-300 group"
                >
                  <Image
                    src={preview}
                    alt={`Preview ${index}`}
                    ...
                    onClick={() => setLightboxImage({ url: preview, index })}
                  />
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full hover:bg-red-600 transition cursor-pointer z-10"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
      ...
      {lightboxImage && (
        ...
        >
              ...
              onClick={(e) => {
                ...
                const newIndex =
                  lightboxImage.index > 0
                    ...
                    : imagePreviews.length - 1;
                setLightboxImage({ url: imagePreviews[newIndex], index: newIndex });
              }}
          ...
          {imagePreviews.length > 1 && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                const newIndex =
                  lightboxImage.index < imagePreviews.length - 1
                    ? lightboxImage.index + 1
                    : 0;
                setLightboxImage({ url: imagePreviews[newIndex], index: newIndex });
              }}
          ...
          {/* Thumbnails */}
          {imagePreviews.length > 1 && (
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 overflow-x-auto max-w-[90vw] pb-2">
              {imagePreviews.map((preview, idx) => (
                <div
                  key={idx}
                  onClick={(e) => {
                    e.stopPropagation();
                    setLightboxImage({ url: preview, index: idx });
                  }}
                  className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 cursor-pointer transition-all bg-white ${
